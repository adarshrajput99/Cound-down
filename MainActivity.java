package com.example.myapplication;

import androidx.appcompat.app.AppCompatActivity;

import android.media.MediaPlayer;
import android.os.Bundle;
import android.os.CountDownTimer;
import android.util.Log;
import android.view.View;
import android.widget.SeekBar;
import android.widget.TextView;

public class MainActivity extends AppCompatActivity {
    SeekBar timeSeeker;
    TextView textView;
    CountDownTimer countDownTimer;
    public void update(int secondsLeft){
        int minutes = secondsLeft / 60;
        int seconds = secondsLeft - (minutes * 60);
        String original;
        if (secondsLeft%60<10){
            original = minutes + ":0"+ seconds;
        }
        else{
            original = minutes + ":"+ seconds;
        }
        textView.setText(original);
    }
    public void startTimer(long seconds){
        textView=findViewById(R.id.textView);
        countDownTimer =new CountDownTimer(timeSeeker.getProgress() * 1000 + 100, 1000) {
            @Override
            public void onTick(long l) {
               update((int) l/1000);
               Log.i("Seconds",String.valueOf(l));
            }

            @Override
            public void onFinish() {
                MediaPlayer mplayer = MediaPlayer.create(getApplicationContext(), R.raw.sound);
                mplayer.start();
            }
        }.start();
    }


    public void buttonClicked(View view){
        startTimer(timeSeeker.getProgress());
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        textView=findViewById(R.id.textView);
        timeSeeker =findViewById(R.id.timeSeekBar);
        timeSeeker.setMax(600);
        timeSeeker.setProgress(30);
        timeSeeker.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int i, boolean b) {
                Log.i("Something changed",String.valueOf(i));
               update(i);
            }

            @Override
            public void onStartTrackingTouch(SeekBar seekBar) {

            }

            @Override
            public void onStopTrackingTouch(SeekBar seekBar) {

            }
        });





    }
}